...
def get_pdf_path(id):
    """ Get the path_to_pdf of a document.
    """
    def method(sess):
        path = sess.query(Document).filter_by(document_id=id).first().path_to_pdf
        return str(path)
...
def get_text_path(id):
    """ Get the path_to_text of a document.
    """
    def method(sess):
        path = sess.query(Document).filter_by(document_id=id).first().path_to_text
        return str(path)
